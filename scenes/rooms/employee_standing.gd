...
func _on_body_entered(body: Node2D) -> void:
	if body.is_in_group("player") and Globals.employee == "unused":
		zone = true
		interact_text.show()
...
func _process(_delta: float) -> void:
	if zone and Input.is_action_just_pressed("interact") and Globals.employee == "unused":
